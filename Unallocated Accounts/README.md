...
| `Claude_dispatch_prompts_-_Fuel_Cards_FTC.md` | The two Claude dispatch prompts for working Section 2 in Envizi - a read-only survey of each FTC account's records against the supplier feed at its job, then the move / move-and-close pass, plus a combined one-account-at-a-time form (prompt 3, all 91 in one run) that reports as it goes. Carries the closing rules and the `_closed` rename convention. |
